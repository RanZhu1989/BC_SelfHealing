...
class Agent:
    ...
    def get_expert_policy(self, env, s0):
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            ...
            s0_pq = expert_varcon_trajectory[step - 1][0]
            s0_pq = tuple(s0_pq)
            a1_varcon = expert_varcon_trajectory[step - 1][1]
            if s0_pq not in self.expert_low[a1_tieline].keys():
                self.expert_low[a1_tieline][s0_pq] = []
            self.expert_low[a1_tieline][s0_pq].append(a1_varcon)
    ...
    def policy_hybrid(self, env, s_high, s_low, mode):
        ...
        if mode == "train" and self.warmstart == True:
            ...
            action['varcon'] = self.expert_low[ah][key_low][0]
            if self.if_logging == True:
    ...
    def run_train(self, env, s0):
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            ...
            s0_line = tuple(s0_line)
            s0_pq = tuple(s0_pq)
            a_gym = self.policy_hybrid(env, s0_line, s0_pq, mode='train')
            ...
            self.nn_varcon[a1_tieline].collect(s0_pq, a1_varcon)
            if self.if_logging == True:
    ...
    def run_test(self, env, s0):
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            a_gym = self.policy_hybrid(env, s0_line, s0_pq, mode='test')
            s_gym, _, _, _ = env.step(a_gym)
            ...
            if self.if_logging == True:
                self.logger.info("System tie-line status: {}".format(s0_line[-5:]))
                # self.logger.info("System tie-line p flow: {}".format(s0_line[-5]))
                # self.logger.info("System tie-line q flow: {}".format(s0_line[-5]))
...
if __name__ == "__main__":
    ...
    output_path = os.getcwd()
    output_path = output_path + "/results/results_BC_hybrid_stochastic_dist/n_4_feature_flow/"
    ...
    for i in trajectory_var0['action'].keys():
        if sum(trajectory_var0['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 0 Usage {}".format(voltage_step_count / (NUM_TOTAL_EPISODES * 5)))
    ...
    for i in trajectory_var1['action'].keys():
        if sum(trajectory_var1['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 1 Usage {}".format(voltage_step_count/(NUM_TOTAL_EPISODES * 5)))
    ...
    for i in trajectory_var2['action'].keys():
        if sum(trajectory_var2['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 2 Usage {}".format(voltage_step_count / (NUM_TOTAL_EPISODES * 5)))
    ...
    for i in trajectory_var3['action'].keys():
        if sum(trajectory_var3['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 3 Usage {}".format(voltage_step_count / (NUM_TOTAL_EPISODES * 5)))
    ...
    for i in trajectory_var4['action'].keys():
        if sum(trajectory_var4['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 4 Usage {}".format(voltage_step_count / (NUM_TOTAL_EPISODES * 5)))
    ...
    for i in trajectory_var5['action'].keys():
        if sum(trajectory_var5['action'][i]) >= 0.01:
            voltage_step_count = voltage_step_count + 1.0
    print("Var 5 Usage {}".format(voltage_step_count / (NUM_TOTAL_EPISODES * 5)))

    # # print('==================== saving nn policy =====================')
    # agent.nn_tieline.model.save(output_path + "DNN_BC_hybrid_tieline" + agent.dt_string + '.h5')
    # agent.nn_varcon[0].model.save(output_path + "DNN_BC_hybrid_var0" + agent.dt_string + '.h5')
    # agent.nn_varcon[1].model.save(output_path + "DNN_BC_hybrid_var1" + agent.dt_string + '.h5')
    # agent.nn_varcon[2].model.save(output_path + "DNN_BC_hybrid_var2" + agent.dt_string + '.h5')
    # agent.nn_varcon[3].model.save(output_path + "DNN_BC_hybrid_var3" + agent.dt_string + '.h5')
    # agent.nn_varcon[4].model.save(output_path + "DNN_BC_hybrid_var4" + agent.dt_string + '.h5')
    # agent.nn_varcon[5].model.save(output_path + "DNN_BC_hybrid_var5" + agent.dt_string + '.h5')
