...
class Agent:
    def __init__(self, env, output_path, log_level=logging.DEBUG):
        ...
        self.nn_varcon = [DNN_VarCon(num_line, num_varcon) for index in range(num_tieline + 1)]
    ...
    def get_expert_policy(self, env, s0):
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            ...
            expert_varcon_trajectory.append((s0_line, a1_varcon))
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            ...
            s0_line = expert_varcon_trajectory[step - 1][0]
            s0_line = tuple(s0_line)
            a1_varcon = expert_varcon_trajectory[step - 1][1]
            if s0_line not in self.expert_low[a1_tieline].keys():
                self.expert_low[a1_tieline][s0_line] = []
                self.logger.info("Record low-level action: {}".format(s0_line))
            self.expert_low[a1_tieline][s0_line].append(a1_varcon)
    ...
    def run_train(self, env, s0):
        ...
        for step in env.exploration_seq_idx[-env.exploration_total:]:
            ...
            s0_line = tuple(s0_line)
            s0_load = tuple(s0_load)
            a_gym = self.policy_hybrid(env, s0_line, s0_line, mode='train')
            ...
            self.nn_varcon[a1_tieline].collect(s0_line, a1_varcon)
            if self.if_logging == True:
...
if __name__ == "__main__":
    ...
    output_path = os.getcwd()
    output_path = output_path + "/results/results_BC_hybrid_stochastic_dist/n_2_feature_line/"
